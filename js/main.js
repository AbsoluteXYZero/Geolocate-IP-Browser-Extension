...
function updateTitle() {
    let parts = [];
    if (currentIPv4) parts.push('IPv4: ' + currentIPv4);
    if (currentIPv6) parts.push('IPv6: ' + currentIPv6);
    chrome.action.setTitle({ title: parts.length ? parts.join('\n') : 'IP Address & Geolocation' });
}
...
async function fetchGeoLocation() {
    ...
    geoLocate6.fetch({
        success: function () {
            currentIPv6 = geoLocate6.get('geoLocation').ipAddress || null;
            updateTitle();
